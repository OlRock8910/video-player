/*
  ...
  Emoji were the first cut and they look wrong: the platform renders 🔀 and 🔁 in
  full colour next to monochrome glyphs, and each font draws them at a different
  weight. These are 24px paths, so everything sits on one grid and inherits the
  current text colour.
*/
...
/** Replaces a button's contents with one icon. */
export function setIcon(node, name, size = 20) {
  if (!node) return;
